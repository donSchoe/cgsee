...
void AssimpLoader::parseMaterials(aiMaterial **materials, const unsigned int numMaterials)
{
    ...
    const aiTextureType TEXTURE_ASSIMP_TYPES[] = {aiTextureType_DIFFUSE, aiTextureType_SPECULAR, aiTextureType_AMBIENT,  aiTextureType_EMISSIVE, aiTextureType_NORMALS, aiTextureType_SHININESS};
    ...
    Texture2D_Type TEXTURE_CGSEE_TYPES[] = {Texture2D_Diffuse, Texture2D_Specular, Texture2D_Ambient, Texture2D_Emission, Texture2D_Normals, Texture2D_Shininess};

    for(int m = 0; m < numMaterials; m++) {
        aiMaterial *aMaterial = materials[m];
        Material *material = new Material;
        qDebug() << "Loading Material " << m;
        // Try to load textures
        for(int t = 0; t < numTextureTypes; t++) {
            if(aMaterial->GetTextureCount(TEXTURE_ASSIMP_TYPES[t]) > 0) {
                qDebug() << "Material has " << TEXTURE_NAMES[t];
                Texture2D *tex = loadTexture(aMaterial, TEXTURE_CGSEE_TYPES[t], TEXTURE_ASSIMP_TYPES[t]);
                if(tex == nullptr) {
                ...
                qDebug() << "Material has no " << TEXTURE_NAMES[t];
            }
        }
        ...
        if(material->attribute<Texture2D>(DIFFUSE_TEX_NAME) == NULL && material->attribute<Texture2D>(AMBIENT_TEX_NAME) != NULL) {
            Texture2D *diffuseTex = loadTexture(aMaterial, Texture2D_Ambient, aiTextureType_AMBIENT);
        ...
        if(material->attribute<Texture2D>(AMBIENT_TEX_NAME) == NULL && material->attribute<Texture2D>(DIFFUSE_TEX_NAME) != NULL) {
            Texture2D *ambientTex = loadTexture(aMaterial, Texture2D_Diffuse, aiTextureType_DIFFUSE);
...
Texture2D *AssimpLoader::loadTexture(aiMaterial *material, Texture2D_Type cgseeType, aiTextureType assimpType)
{
    Texture2D *tex;
    aiString aTexPath;
    QString texPath;

    material->GetTexture(assimpType, 0, &aTexPath);
