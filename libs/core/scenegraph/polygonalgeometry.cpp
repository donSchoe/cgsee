...
GLuint PolygonalGeometry::maxIndex() const
{
    t_uints indices = this->indices();
    GLuint max = 0;
...
void PolygonalGeometry::initialize(const Program & program)
{
    ...
    glGenVertexArrays(1, &m_vao); glError();
    glBindVertexArray(m_vao); glError();
    ...
    if(!normals().isEmpty()) {
        ...
        normalBO->noData<glm::vec3>(glm::vec3(0.0), maxIndex() + 1, GL_FLOAT, 3);
    }
    ...
    if(!texcs().isEmpty()) {
        ...
        texcsBO->noData<glm::vec2>(glm::vec2(0.0), maxIndex() + 1, GL_FLOAT, 2);
    }
